...
{
    partial class PharmixJobService : ServiceBase
    {
        Timer timer = new Timer();
        Common common = new Common();
        List<UploadedFiles> lstuploadedFiles = new List<UploadedFiles>();
        PharmixWebApiClient pPharmixWebApiClient = new PharmixWebApiClient();
        ...
        protected override void OnStart(string[] args)
        {
            ...
            TraceService("Job is started at " + DateTime.Now);
            UnzipSourceFile();
            TraceService("Job is ended at " + DateTime.Now);
        ...
        private void OnElapsedTime(object source, ElapsedEventArgs e)
        {
            ...
            if (IsCompleted)
            {
                ...
                TraceService("Job is started at " + DateTime.Now);
                UnzipSourceFile();
                TraceService("Job is ended " + DateTime.Now);
            }
        }
        ...
        public void AddUploadedFilesWebAi(string changeSetId)
        {
            ...
            HttpClient client = pPharmixWebApiClient.InitializeClient();
        ...
        public void EmailBody(DataRow item, string filePath)
        {
            TraceService("******************************** EmailBody is started  " + DateTime.Now + " ******************************");            

            string messageBody = string.Empty;
            using (StreamReader reader = new StreamReader(ConfigurationManager.AppSettings["TemplatePath"].ToString()))
            {
                messageBody = reader.ReadToEnd();
            }
           
            messageBody = messageBody.Replace("{EMAIL_NAME}", item[8].ToString());
            messageBody = messageBody.Replace("{EMAIL_DOWNLOAD_LINK}", filePath);

            new Email().SendEmail(item[1].ToString(), "Weekly Changesets", messageBody, filePath);
